...
/**
 * Утилита Serde для Avro-записей {@link SpecificRecord} без Schema Registry.
 *
 * Класс схемы известен заранее на стороне читателя, поэтому достаточно чистого
 * Avro-бинаря (без префикса schema-id, как в Confluent wire format).
 */
public final class AvroSerdes {
	...
	/**
	 * Сериализатор: SpecificRecord → бинарь через {@link org.apache.avro.io.BinaryEncoder}.
	 */
	...
	/**
	 * Десериализатор: бинарь → SpecificRecord по известному классу схемы.
	 */
	public static class AvroSerdeDeserializer<T extends SpecificRecord> implements Deserializer<T> {
		...
		public T deserialize(String topic, byte[] data) {
			...
			try {
				...
				var reader = new SpecificDatumReader<T>(prototype.getSchema());
